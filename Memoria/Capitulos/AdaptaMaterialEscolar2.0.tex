...
En esta sección se explicará los diseños creados por cada integrante junto a las imágenes de los mismos. Todos los diseños mencionados se encuentran en el Anexo (Capítulo \ref{cap:anexo}), en las secciones \ref{sec:disenyoAlvaro}, \ref{sec:disenyoDunia}, \ref{sec:disenyoAlberto}, \ref{sec:disenyoJohan}.
...
El diseño que realicé es bastante parecido al de la aplicación ya existente. Intenté mantener la misma estructura y funcionalidad pero haciendo que la interfaz de usuario sea más profesional. Para esto ajusté el diseño de la barra de navegación, los botones, el componente para seleccionar un fichero pdf, etc. En la pantalla principal tenemos la barra de navegación y un componente en el que podemos arrastrar o seleccionar un fichero pdf. Una vez seleccionado el archivo, nos lleva a otra página que se divide en dos, en una mitad tenemos el pdf y en la otra tenemos el editor de texto. Estos diseños se pueden observar en las figuras \ref{AlbertoPaginaPrincipal1} y \ref{AlbertoPaginaPrincipal2}. Por último, el usuario podrá acceder a cada funcionalidad haciendo click en los botones que se encuentran en la parte superior del editor.

Para hacer el diseño de las funcionalidades ya implementadas en AdaptaMaterial 1.0 también he utilizado la misma estructura, haciendo algunos ajustes para darles un estilo más profesional, pero manteniendo el mismo funcionamiento. En las figuras \ref{Alberto13}, \ref{Alberto14}, \ref{Alberto15}, \ref{Alberto16} y \ref{Alberto17} se muestran los diseños de estas funcionalidades.

En cuanto a las nuevas funcionalidades que no estaban implementadas en AdaptaMaterial 1.0, realicé los siguientes diseños:

\begin{itemize}
  \item \textbf{Leyenda de colores:} tenemos un input en el que se puede escribir el nombre de la categoría y un selector de colores para poder elegir el color que le corresponde. Una vez hecho esto se puede hacer click en el botón con el símbolo de más para añadir la categoría a la lista. También tenemos una vista previa en la que podemos ver cómo será el resultado. Este diseño se puede ver en la figura \ref{Alberto1}.
  ...
  \item \textbf{Cuadrícula y doble pauta:} estas dos funcionalidades tienen un modal igual, en el que simplemente seleccionamos el número de líneas que queremos insertar en el documento de trabajo. Este diseño se puede ver en las figuras \ref{Alberto3} y \ref{Alberto4}.
  ...
  \item \textbf{Fórmulas matemáticas:} para este ejercicio simplemente añadí un input en el que se pueda escribir una fórmula matemática en LaTeX. Este diseño se puede ver en la figura \ref{Alberto6}.
  \item \textbf{Generar resumen y pictotraductor:} estas dos funcionalidades también tienen una interfaz igual, simplemente hay un input en el que el usuario puede pegar el contenido que quiere resumir o traducir a pictogramas y al dar click en el botón de aceptar, se generará automáticamente a través de una API y se insertará el resultado en el documento de trabajo. Este diseño se puede ver en las figuras \ref{Alberto8} y \ref{Alberto10}.
  \item \textbf{Espacio para dibujar:} tenemos dos inputs, en los cuales podemos escribir la altura y anchura en centímetros que va a tener el espacio para dibujar. Al insertarse en el documento de trabajo, este espacio será simplemente un rectángulo vacío con un borde para delimitar el espacio. Este diseño se puede ver en la figura \ref{Alberto7}.
  \item \textbf{Buscar imágenes:} el funcionamiento es igual al de la funcionalidad de buscar pictogramas, el usuario puede hacer una búsqueda y se le mostrarán todos los resultados encontrados y al darle click a uno de ellos, esta imagen se insertará en el documento de trabajo. Este diseño se puede ver en la figura \ref{Alberto12}.
\end{itemize}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=0.7\textwidth]{Diseño/definicionesAE1.PNG}
...
En este TFG hemos decidido usar Slate, una biblioteca de JavaScript que se utiliza para contruir editores de texto enriquecido. Lo hemos combinado con React lo que nos permite crear una aplicación basada en componentes que a largo plazo será más escalable. Ambas herramientas se apoyan en el patrón \textit{Composite}\footnote{\url{https://refactoring.guru/es/design-patterns/composite}} el cual permite generar un componente complejo mediante la anidación de componentes simples. Dicho patrón se encuentra integrado en Slate lo que nos facilita la configuración del mismo. En cuanto a la estructura interna de React ayuda a implementarlo, por nuetra parte lo hemos empleado para el diseño de los modales. Por otro lado, también hemos usado el patrón creacional \textit{Factory}\footnote{\url{https://refactoring.guru/es/design-patterns/factory-method}} que nos permite crear objetos sin tener que conocer los detalles de su creación, lo hemos usado para la creación de los modales y de las distintas barras de herramientas que se encuentran en el editor.

En cuento a cómo se encuentran conectados Slate y React cabe mencionar que hemos creado un conjunto de funciones que hacen de intermediario entre los modales y el editor.