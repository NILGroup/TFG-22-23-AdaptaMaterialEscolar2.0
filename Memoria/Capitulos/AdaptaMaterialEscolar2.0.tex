...
A continuación se muestran los diseños de las nuevas funcionalidades que no estaban implementadas en AdaptaMaterialEscolar 1.0:
...
\begin{itemize}
  ...
  \item Texto: Esta barra de herramientas contiene opciones para manipular el texto. Todas, al igual que la anterior barra de herramientas, despliegan un modal para obtener la información necesaria para crear el nodo:
        \begin{itemize}
          ...
          \item Picotraductor: Esta función obtiene una serie de imagenes, a traves de la API de PICTAR, en base a la información obtenida. Estas imagenes se introducen en el nodo ``pictotranslator''. Despues esto se introduce dentro del nodo ``bloqueEditable''
          \item Leyenda de colores: Con la información obtenida se introduce tantos parrafos comno parejas de color y concepto, haya introducido el usuario, dentro del nodo ``bloqueEditable''.  
          \item Buscar picotgramos: Esta función obtiene una imagen mediante la API de ARASAAC a través de la información proporcionada por el usuario. Esta imagen se introduce en un nodo que hemos denominado ``image''.
        \end{itemize}
\end{itemize}
