...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1\textwidth]{Editor/Archivo.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1\textwidth]{Editor/Formato.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1\textwidth]{Editor/Ejercicios.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1\textwidth]{Editor/Texto.png}
...
\begin{lstlisting}[label=fig:impsopaletrasopciones, caption=Opciones para la sopa de letras., language=JavaScript]
...
Al pulsar el botón de resumir se realiza una petición HTTP de tipo POST a la ruta ``/summary'', enviando el texto a resumir y el número de palabras del resumen. En el \textit{backend} se llama a la función \textit{createChatCompletion} de la clase \textit{OpenAIApi}, utilizando ``Resume este texto en aproximadamente {numeroPalabras} palabras, si no se puede resumir el texto devuelve -1: {texto}'' como \textit{prompt}, con el número de palabras y el texto introducidos por el usuario y se devuelve el resumen generado al cliente. El resumen se mostrará en el panel de vista previa. En caso de que no se pueda generar el resumen, o el texto original sea muy breve, se mostrará un error al usuario en la vista previa. Al pulsar el botón de \textit{Ok}, se cerrará el modal y se insertará el texto resumido en el documento de trabajo. En la Figura \ref{fig:impresumen} se muestra la funcionalidad de generar resumen.
