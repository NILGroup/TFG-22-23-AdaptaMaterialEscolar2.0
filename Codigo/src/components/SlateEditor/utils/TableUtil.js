...
export class TableUtil {
	...
	insertTable = (celltext, rows = 0, columns = 0) => {
		...
		let newTable;
		if (rows > 0 && columns > 0) newTable = createTableNode(rows, columns);
		else newTable = createTableNodeByArray(celltext);
...
const createTableNode = (rows, columns) => {
...
// Funcion para crear el nodo tabla con celdas vacias
const createTableNodeByArray = (celltext) => {
	const tableChildren = Array.from(celltext, (value) => createRow(value));
