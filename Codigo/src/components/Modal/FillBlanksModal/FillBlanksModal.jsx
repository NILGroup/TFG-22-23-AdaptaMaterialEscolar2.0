        ...
        <Modal title={"Completar Huecos"} isOpen={isOpen} onClose={closeModal}>
            <form
                ...
                onSubmit={(e) => {
                    ...
                    Transforms.insertNodes(editor, { type: 'paragraph', children: [{ text: exercise }] });
                ...
                <div className={style.modalFormGroup}>
                    ...
                    {isAddingGaps ?
                        <>
                            <p className={`${style.modalTooltip} ${style.modalCenter}`}>
                            ...
                            <div className={style.modalTextArea}>
                                {words?.map((word, index, array) => {
                                    ...
                                    return (
                                        <span key={`word-${index}`} className={style.modalInteractable} onClick={(e) => {
                                            if (e.target.getAttribute("data-gap")) {
                                                const word = e.target.getAttribute("data-gap");

                                                e.target.removeAttribute("data-gap");
                                                ...
                                                e.target.setAttribute("data-gap", `${e.target.innerText}`)

                                                const gap = "_".repeat(gapInfo.length);
                        ...
                        :
                        <textarea name="originalText" id="originalText" className={style.modalTextArea} onChange={handleInput} value={originalText} />
                    }
                    <button
                ...
                <div className={style.modalFormGroup}>
                    ...
                    <ModalGapRadio gapType={GapType.Medium} defaultChecked={true} onChange={setGapType} />
                    <ModalGapRadio gapType={GapType.Big} onChange={setGapType} />
                </div>
                <button type="submit" className={`${style.modalButton} ${style.modalCenter}`} disabled={words === null}>Ok</button>
            </form>
        </Modal>
    );
}
