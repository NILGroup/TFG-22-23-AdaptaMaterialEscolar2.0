...
export default function SearchPictoModal({ editor, isOpen, onClose }) {
    ...
    return (
        <Modal title={"Buscar Pictograma"} isOpen={isOpen} onClose={closeModal}>
            <form className={style.modalForm} onSubmit={(e) => {
                e.preventDefault();
                ...
                <div className={style.modalFormGroup}>
                    <label htmlFor="searchPictogram" className={style.modalLabelHeading}>Buscador</label>
                    <input type="text" name="searchPictogram" id="searchPictogram" className={style.modalInput} required />
                </div>
                <input type="submit" value="Buscar" className={`${style.modalButton} ${style.modalCenter}`} />
            </form>
            {pictogramList && <hr className={style.modalHorizontalRule} />}
            {isLoading ? <Spinner /> : <ModalPictogramList editor={editor} pictograms={pictogramList} callback={closeModal} />}
        </Modal>
    );
}
